 ...
 * @note  DMA_BURST_1_WORD by default.
 */
typedef enum
{
    DMA_BURST_1_WORD = 0,
...
typedef enum
{
    ...
    DMA_CHN8_IRQ  = BIT(8),
    DMA_CHN9_IRQ  = BIT(9),
    DMA_CHN10_IRQ = BIT(10),
    DMA_CHN11_IRQ = BIT(11),
    DMA_CHN12_IRQ = BIT(12),
    DMA_CHN13_IRQ = BIT(13),
    DMA_CHN14_IRQ = BIT(14),
    DMA_CHN15_IRQ = BIT(15),
} dma_irq_chn_e;
...
typedef enum
{
    ...
    DMA_REQ_NVM_TX = DMA_REQ_RRAM_TX,
    DMA_REQ_NVM_RX = DMA_REQ_RRAM_RX,
...
{
    unsigned char dst_req_sel = ((*(unsigned int *)config) >> 2) & 0x3f;
    unsigned char src_req_sel = ((*(unsigned int *)config) >> 8) & 0x3f;
    unsigned char req_sel     = (dst_req_sel != 0) ? dst_req_sel : src_req_sel;

    if (chn >> 3) {
        (req_sel < 32) ? (reg_dma_set_l |= BIT(req_sel)) : (reg_dma_set_h |= BIT(req_sel - 32));
    } else {
        (req_sel < 32) ? (reg_dma_set_l &= ~(BIT(req_sel))) : (reg_dma_set_h &= ~(BIT(req_sel - 32)));
    }
...
static inline void dma_set_irq_mask(dma_chn_e chn, dma_irq_mask_e mask)
{
    ...
    if (mask & ERR_MASK)
    {
        reg_dma_err_mask(chn) |= BIT(chn % 8);
    }
    if (mask & ABT_MASK)
    {
        reg_dma_abt_mask(chn) |= BIT(chn % 8);
    }
}
...
static inline _Bool dma_is_irq_mask(dma_chn_e chn, dma_irq_mask_e mask)
{
    ...
    case TC_MASK:
        ...
        break;
    case ERR_MASK:
        mask_flag = reg_dma_err_mask(chn) & BIT(chn % 8);
        break;
    case ABT_MASK:
        mask_flag = reg_dma_abt_mask(chn) & BIT(chn % 8);
        break;
    default:
        break;
    }
...
{
    return reg_dma_tc_isr(tc_chn) & (tc_chn > 0x80 ? (tc_chn >> 8) : tc_chn);
}
...
{
    reg_dma_tc_isr(tc_chn) = tc_chn > 0x80 ? (tc_chn >> 8) : tc_chn;
}
...
{
    return reg_dma_err_isr(err_chn) & (err_chn > 0x80 ? (err_chn >> 8) : err_chn);
}
...
{
    reg_dma_err_isr(err_chn) = err_chn > 0x80 ? (err_chn >> 8) : err_chn;
}
...
{
    return reg_dma_abt_isr(abt_chn) & (abt_chn > 0x80 ? (abt_chn >> 8) : abt_chn);
}
...
{
    reg_dma_abt_isr(abt_chn) = abt_chn > 0x80 ? (abt_chn >> 8) : abt_chn;
}
...
{
    reg_dma_size0(chn) = ((size_byte + byte_width - 1) / byte_width) & 0xffff;
    reg_dma_size1(chn) = ((((size_byte + byte_width - 1) / byte_width) >> 16) & 0x3f) | ((size_byte % byte_width) << 6);
}
...
/**
 * @brief   This function resets DMA0,the DMA0 logic and registers will be reset.
 * @return    none
 */
...
/**
 * @brief   This function resets DMA1,the DMA1 logic and registers will be reset.
 * @return    none
 */
...
{
    reg_dma_ch_cr3(chn) |= FLD_DMA_W_NUM_EN;
}
...
/**
 * @brief     This function set DMA llp sof mode.
 * @param[in] chn       - dma channel.
...
{
    if (sof_mode) {
        reg_dma_ctrl_llp(chn) |= BIT((chn < 8 ? chn : (chn - 8)));
    } else {
        reg_dma_ctrl_llp(chn) &= ~BIT((chn < 8 ? chn : (chn - 8)));
    }
}
...
/**
 * @brief      This function servers to get dma llp address.
 * @param[in]  chn - dma channel.
 * @return     none
 */
static inline unsigned int dma_get_llp_addr(dma_chn_e chn)
{
    return reg_dma_llp(chn);
}
