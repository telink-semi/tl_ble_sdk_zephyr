 ...
 * @return     none
 * @note       Logic HIGH selects slow slew rate, logic LOW selectsfast slew rate.
 */
void gpio_set_slew_rate(gpio_pin_e pin, GPIO_Drv_Slew_Rate value)
{
    unsigned char  bit   = pin & 0xff;
    unsigned short group = pin & 0xf00;
 ...
 * @note    | DS0 | DS1 | SR | Drv Strength |
            ...
            | 1   | 1   | 0  | 12mA         |
 */
void gpio_set_data_strength(gpio_pin_e pin, GPIO_Drv_Strength value)
{
    unsigned char  bit   = pin & 0xff;
    unsigned short group = pin & 0xf00;
    if (pin == GPIO_PB4 || pin == GPIO_PB5 || pin == GPIO_PB6 || pin == GPIO_PB7) {
