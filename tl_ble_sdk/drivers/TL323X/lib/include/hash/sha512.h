 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int sha512_update(SHA512_CTX *ctx, unsigned char *msg, unsigned int msg_bytes);
 ...
 *        address and byte length.
  @endverbatim
 */
    unsigned int sha512_node_steps(HASH_NODE *node, unsigned int node_num, unsigned char *digest);
    #endif
 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int sha512_dma_update_blocks(SHA512_DMA_CTX *ctx, unsigned int *msg, unsigned int msg_bytes);
 ...
 */
    ...
    /**
 * @brief       input whole message and get its sha512 digest(dma node style)
 * @param[in]   node             - input, message node pointer
 * @param[in]   node_num         - input, number of hash nodes, i.e. number of message segments.
 * @param[out]   digest           - output, sha512 digest, 64 bytes
 * @param[in]   callback         - callback function pointer
 ...
 *     -# 3. for every node or segment except the last, its message length must be a multiple of block length.
  @endverbatim
 */
    unsigned int sha512_dma_node_steps(HASH_DMA_NODE *node, unsigned int node_num, unsigned int *digest, HASH_CALLBACK callback);
        #endif
