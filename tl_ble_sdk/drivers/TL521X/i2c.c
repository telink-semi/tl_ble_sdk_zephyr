...
void i2c_set_pin(gpio_func_pin_e sda_pin, gpio_func_pin_e scl_pin)
{
    ...
    gpio_set_up_down_res((gpio_pin_e)sda_pin, GPIO_PIN_PULLUP_20K);
    gpio_set_up_down_res((gpio_pin_e)scl_pin, GPIO_PIN_PULLUP_20K);
...
unsigned char i2c_master_write(unsigned char id, unsigned char *data, unsigned int len)
{
    ...
    while (cnt < len) {
        ...
        if (clock_time_exceed(start_cycle, g_i2c_error_timeout_us)) {
