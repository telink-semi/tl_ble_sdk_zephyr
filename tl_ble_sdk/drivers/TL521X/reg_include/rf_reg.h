...
enum
{
    ...
    FLD_BB_TIMER_TIMER_ALIGN     = BIT(4),
    FLD_BB_TIMER_TIMER_SYN_MODE  = BIT(5),
    FLD_BB_TIMER_TIMER_PREC_MODE = BIT_RNG(6, 7),
};
...
typedef enum
{
    ...
    FLD_BB_TIMER_CMD_ALIGN_TRIG   = BIT(2), //The system timer alignment function is triggered
} rf_bb_timer_irq_e;
...
#define reg_rf_sys_timer_ctrl2 REG_ADDR8(BB_TIMER_BASE_ADDR + 0x1a)

enum
{
    FLD_BB_TIMER_SYS_TIMER_ALIGN_COMP = BIT_RNG(0, 4), //Compensation of system timer and system timer bb counters. default:0x10
    FLD_BB_TIMER_HOLD                 = BIT(5),        //0x0
};

#define reg_rf_sys_timer_ctrl3 REG_ADDR8(BB_TIMER_BASE_ADDR + 0x1b)

enum
{
    FLD_BB_TIMER_SYS_TIMER_ALIGN_CNT    = BIT_RNG(0, 1), //system timer and system timer bb clock count alignment compensation. default:0x1
    FLD_BB_TIMER_SYS_32KTIMER_ALIGN_CNT = BIT_RNG(2, 3), //system timer and system timer bb 32k clock count alignment compensation. default:0x1
};
...
#define reg_rf_pkt_flt_match_l REG_ADDR32(REG_BASEBAND_BASE_ADDR + 0x64)
...
#define reg_rf_pkt_flt_match_h REG_ADDR32(REG_BASEBAND_BASE_ADDR + 0x68)
...
#define reg_rf_pkt_flt_mask_l REG_ADDR32(REG_BASEBAND_BASE_ADDR + 0x6c)
...
#define reg_rf_pkt_flt_mask_h REG_ADDR32(REG_BASEBAND_BASE_ADDR + 0x70)
...
enum
{
    ...
    FLD_RF_R_RESET_PID_6   = BIT(6),
    FLD_RF_R_RESET_PID_7   = BIT(7),
    FLD_RF_R_RESET_PID_ALL = 0xff,
};
...
#define reg_rf_tstimp_ctrl REG_ADDR8(REG_BB_LL_BASE_ADDR + 0x09)

enum
{
    FLD_RF_R_STIMER_REVERT_EN = BIT(0),  //Set to 1, stimer controls the function and ststamp through the system timer;
                                         //Otherwise system timer bb is used
    FLD_RF_R_ST_HADM_REVERT_EN = BIT(1), //Set to 1, hadm ststamp through the system timer;Otherwise system timer bb is used
};
...
#define reg_rf_radio_tx_irq_mask REG_ADDR8(REG_TL_RADIO_BASE_ADDR + 0x196)
#define reg_rf_radio_rx_irq_mask REG_ADDR8(REG_TL_RADIO_BASE_ADDR + 0x199)
...
typedef enum
{
    ...
    FLD_RF_IRQ_RX_FIFO_FULL      = BIT(3),  /*Auto/Manual:An interrupt is generated when the data size of the total received packets is exceeded to the allocated storage range of Rx SRAM
                                                         TL321X chip unavailable, it will not work after configuration.(add by chenxi.wang, confirmed by jingyong 202402029)*/
    ...
    FLD_RF_IRQ_CALC_TEST_FSMDONE = BIT(28), 
    FLD_RF_IRQ_CALC_YESY_DONE    = BIT(29), 
    FLD_RF_IRQ_CS_INPCT_DONE     = BIT(30), 
    FLD_RF_IRQ_DEC_DONE          = BIT(31), 

    FLD_RF_IRQ_ALL               = 0xffffffff,
} rf_irq_e;
...
enum
{
    ...
    FLD_RF_PID6 = BIT_RNG(4, 5),
    FLD_RF_PID7 = BIT_RNG(6, 7),
};
...
#define reg_rf_ll_dec_dly        REG_ADDR8(REG_BB_LL_BASE_ADDR + 0x2f)

enum
{
    FLD_RF_RX_DEC_DLY    = BIT_RNG(0, 3),
    FLD_RF_RX_DEC_DLY_EN = BIT(4),
    FLD_RF_RX_ULL_EN     = BIT(5),
    FLD_RF_TX_ULL_EN     = BIT(6),
    FLD_RF_RX_P_RST_EN   = BIT(7),
};
...
#define reg_rf_ull_rx_ctrl1 REG_ADDR8(REG_BB_LL_BASE_ADDR + 0xcb)

enum
{
    FLD_RF_RX_DEC_ULL_DLY_EN = BIT(0),
};
...
enum
{
    ...
    FLD_RF_GF_BT        = BIT(2),
    FLD_RF_GF_BT2_0     = BIT(3),
};

#define reg_rf_zb_freq_corr_cfg_1 REG_ADDR8(REG_TL_MODEM_BASE_ADDR + 0x3b)

enum
{
    FLD_RF_ZB_NUM_GEAR_H      = BIT(0),
    FLD_RF_ZB_START_STEP_TIME = BIT_RNG(1, 3),
    FLD_RF_SEIZE_ZB_INIT_DC_ON_COUNT     = BIT(4),
    FLD_RF_RESTART_FREQ_REC_AGC_DISABLE  = BIT(5),
    FLD_RF_ZB_SFD_BYTE_REV    = BIT(6),
};
...
#define reg_rf_hp_demod_cfg0 REG_ADDR8(REG_TL_MODEM_BASE_ADDR + 0x115)

enum
{
    FLD_RF_HP_MULTICEIVER_EN  = BIT(0),
    FLD_RF_HP_PDET_DCO_DLY_EN = BIT(1),
    FLD_RF_HP_RX_CG_MANUAL_EN = BIT(2),
    FLD_RF_HP_PDET_DC_DLY     = BIT_RNG(3, 4),
    FLD_RF_HP_PDET_SYM_OFFSET = BIT_RNG(5, 6),
    FLD_RF_LS_CHF_SEL         = BIT(7),
};
...
enum
{
    FLD_RF_HP_PDET_PWR_THD_L  = BIT_RNG(0, 5),
    FLD_RF_HP_CH1_SYNC_EN     = BIT(6),
    FLD_RF_HP_MULTICEIVER_LEN = BIT(7),
};

#define reg_rf_hp_demod_cfg3 REG_ADDR8(REG_TL_MODEM_BASE_ADDR + 0x118)
#define reg_rf_hp_demod_cfg4 REG_ADDR8(REG_TL_MODEM_BASE_ADDR + 0x119)

enum
{
    FLD_RF_HP_PDET_CORR_THD_10_8 = BIT_RNG(0, 2),
    FLD_RF_HP_PDET_DAGC_LOW_THD  = BIT_RNG(3, 6),
    FLD_RF_HP_DEM_DC_ALPHA_H     = BIT(7),
};

#define reg_rf_hp_demod_cfg10 REG_ADDR8(REG_TL_MODEM_BASE_ADDR + 0x11f)

enum
{
    FLD_RF_HP_LR_DC_ALPHA_S8   = BIT_RNG(0, 3),
    FLD_RF_HP_LR_DMD_OFFSET    = BIT_RNG(4, 5),
    FLD_RF_HP_MULTICEIVER_HALF = BIT(6),
};

#define reg_rf_adc_dac_dft1_2 REG_ADDR8(REG_TL_MODEM_BASE_ADDR + 0x126)

enum
{
    FLD_RF_TX_DATA_OW_IQ_VAL = BIT_RNG(0, 2),
    FLD_RF_TX_DATA_OW_IQ_SEL = BIT(3),
    FLD_RF_PDET_DCO_CORR_THRESHOLD_11 = BIT(7),
};

#define reg_rf_hp_demod_cfg13 REG_ADDR8(REG_TL_MODEM_BASE_ADDR + 0x127)

enum
{
    FLD_RF_PDET_DCO_CORR_THRESHOLD_19_12 = BIT_RNG(0, 7),
};

#define reg_rf_hp_demod_cfg14 REG_ADDR8(REG_TL_MODEM_BASE_ADDR + 0x128)

enum
{
    FLD_RF_RX_FREQOFF_LIMIT_MAX_7_0 = BIT_RNG(0, 7),
};

#define reg_rf_hp_demod_cfg15 REG_ADDR8(REG_TL_MODEM_BASE_ADDR + 0x129)

enum
{
    FLD_RF_RX_FREQOFF_LIMIT_MAX_12_8 = BIT_RNG(0, 4),
    FLD_RF_RX_FREQOFF_LIMIT_EN       = BIT(5),
    FLD_RF_RX_DAGC_SFT_SEL           = BIT(6),
};

#define reg_rf_clk_sample_sel REG_ADDR8(REG_TL_MODEM_BASE_ADDR + 0x13a)

enum
{
    FLD_RF_CLK_SAMPLE_SEL       = BIT(0),
    FLD_RF_MANUAL_SYNCWORD      = BIT(1),
    FLD_RF_R_SHIIFT_CHF_SEL     = BIT(2),
    FLD_RF_R_LIMTRE_FILTER_M128 = BIT(3),
    FLD_RF_R_PHASE_EN           = BIT(4),
    FLD_RF_DC_EST_SYNC_LOCK     = BIT(5),
    FLD_RF_DC_EST_WIN_SIZE_SEL  = BIT(6),
};

#define reg_rf_tx_rtt_0 REG_ADDR8(REG_TL_MODEM_BASE_ADDR + 0x13c)

enum
{
    FLD_RF_TX_RTT_EN = BIT(0),
    FLD_RF_TX_GAUSS_SAMPLE_FIXED = BIT(1),
};
...
enum
{
    ...
    FLD_RF_TX_PA_RAMP_DV2_EN = BIT(5),
};
...
#define reg_rf_radio_extral_1 REG_ADDR8(REG_TL_RADIO_BASE_ADDR + 0x39)

enum
{
    FLD_RF_PA_CAS_BIAS_DIG = BIT_RNG(0, 1),
    FLD_RF_TX_MI_FLEX_HP_O = BIT(2),
};
...
#define reg_rf_txrx_dbg4_0 REG_ADDR8(REG_TL_RADIO_BASE_ADDR + 0x46)

enum
{
    FLD_RF_TXC_LPM_DELAY         = BIT_RNG(0, 2),
    FLD_RF_AGC_SRSSI_COMP_SEL    = BIT(3),
    FLD_RF_CH_POW_CHNG_RST_DLY_L = BIT_RNG(5, 7),
};
...
#define reg_rf_hshp_ctrl_0                REG_ADDR8(REG_TL_RADIO_BASE_ADDR + 0x1e0)

enum
{
    FLD_RF_RX_HS_48M_EN       = BIT(0),
    FLD_RF_RX_HP_DEMOD_EN     = BIT(1),
    FLD_RF_RXC_MODE_SEL       = BIT_RNG(2, 3), //48M RF demod rate sel(0:1mbps 1:2mbps 2:4mbps 3:6mbps)
    FLD_RF_REG_CLK_DIG_48_DIG = BIT(4),
};

#define reg_rf_hshp_ctrl_1 REG_ADDR8(REG_TL_RADIO_BASE_ADDR + 0x1e1)

enum
{
    FLD_RF_TX_HS_48M_EN = BIT(0),
    FLD_RF_TXC_MODE_SEL = BIT_RNG(2, 3), //48M RF demod rate sel(0:1mbps 1:2mbps 2:4mbps 3:6mbps)
    FLD_RF_TX_LS_24M_EN = BIT(4),
};

#define reg_rf_hshp_ctrl_2 REG_ADDR8(REG_TL_RADIO_BASE_ADDR + 0x1e2)

enum
{
    FLD_RF_RXC_MODE_OW      = BIT(0),
    FLD_RF_TXC_LPM_FD_HS_OW = BIT(1),
    FLD_RF_RXC_SWRST_HS_OW  = BIT(2),
};

#define reg_rf_hshp_ctrl_3 REG_ADDR8(REG_TL_RADIO_BASE_ADDR + 0x1e3)

enum
{
    FLD_RF_RXC_MODE_VAL      = BIT(0),
    FLD_RF_TXC_LPM_FD_HS_VAL = BIT(1),
    FLD_RF_RXC_SWRST_HS_VAL  = BIT(2),
    FLD_RF_AUTO_PHY_EN       = BIT(4),
    FLD_RF_TX_IF_FREQ_ASYM   = BIT_RNG(5, 6),
};

#define reg_rf_hshp_ctrl_4 REG_ADDR8(REG_TL_RADIO_BASE_ADDR + 0x1e4)

enum
{
    FLD_RF_TXC_NARROW_BANDWIDTH0      = BIT(0),
    FLD_RF_TXC_NARROW_BANDWIDTH1      = BIT(1),
    FLD_RF_RX_ULL_MODE                = BIT(2),
    FLD_RF_TX_ULL_MODE                = BIT(3),
    FLD_RF_RX_POW_DET_LAT_EN          = BIT(4),
    FLD_RF_RX_POW_CHG_RST_EN          = BIT(5),
    FLD_RF_RX_RESTART_FREQ_REC_EN     = BIT(6),
    FLD_RF_RX_RESTART_FREQ_REC_RST_EN = BIT(7),
};

#define reg_rf_pwr_restart_ctrl REG_ADDR8(REG_TL_RADIO_BASE_ADDR + 0x1e5)

enum
{
    FLD_RF_RX_RESTART_FREQ_REC_DLY    = BIT_RNG(0, 4),
    FLD_RF_RX_RESTART_FREQ_REC_DLY_EN = BIT(5),
    FLD_RF_RX_ULL_SEQ                 = BIT(6),
    FLD_RF_TX_ULL_SEQ                 = BIT(7),
};
