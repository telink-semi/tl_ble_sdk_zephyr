...
#if (FREERTOS_ENABLE && ((__PROJECT_SNIF_MAIN_NODE__==1) || (__PROJECT_CS_INITIATOR_DEMO__==1)))
 ...
 * @return      none
 */
_attribute_ram_code_sec_noinline_ void tlkapi_debug_handler(void)
{
    #if (TLKAPI_DEBUG_CHANNEL == TLKAPI_DEBUG_CHANNEL_UDB)
    udb_usb_handle_irq();
    #elif (TLKAPI_DEBUG_CHANNEL == TLKAPI_DEBUG_CHANNEL_GSUART)
 ...
 * @param   pData hex data
 * @param   data_len the length of the hex data
*/
_attribute_ram_code_sec_noinline_ void tlkapi_send_str_data(char *str, u8 *pData, u32 data_len)
{
    if (!tlkapi_print_fifo) {
        return;
    }
    ...
    if (tlkDbgCtl.dbg_chn == TLKAPI_DEBUG_CHANNEL_UDB) {
        /**
         * @note UDB_frame_struct {
         *          u8 header[3];
         *          u8 data_len[2]; // equals to hex data len
         *          u8 data[];
         *          u8 strings[];
         *          };
         *
         * @note if len == 64, an empty packet needs to be added to
...
_attribute_ram_code_sec_noinline_ void tlkapi_send_str_u8s(char *str, int size, ...)
{
...
_attribute_ram_code_sec_noinline_ void tlkapi_send_str_u32s(char *str, int size, ...)
{
