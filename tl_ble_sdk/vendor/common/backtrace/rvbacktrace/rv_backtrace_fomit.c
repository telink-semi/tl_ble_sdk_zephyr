 ...
 * 2025-04-13     Supper Thomas fix assembly instruction exception, change immediate to 8-byte alignment for RISC-V32 platform adaptation
 */
...
extern char *BACKTRACE_RAM_CODE_START;
extern char *BACKTRACE_RAM_CODE_END;
...
#define BT_CHK_PC_AVAIL(pc)   (((uintptr_t)(pc) < (uintptr_t)(&BACKTRACE_TEXT_END) \
                              ...
                              || ((uintptr_t)(pc) > (uintptr_t)(&BACKTRACE_RAM_CODE_START) \
                              && (uintptr_t)(pc) < (uintptr_t)(&BACKTRACE_RAM_CODE_END)))
...
static int riscv_backtrace_framesize_get1(unsigned int inst)
{
    ...
    if ((inst & 0x800FFFFF) == 0x80010113) {
        ...
        imm = (~imm & 0x7FF) + 1;
#if __riscv_xlen == 64
        return imm >> 3; // RV64: in 8-byte units
#else
        return imm >> 2;  // RV32: in 4-byte units
#endif
    }
...
static int riscv_backtrace_framesize_get(unsigned short inst)
{
    ...
    */
    if ((inst & 0xFF83) == 0x7101) {
        ...
        imm = ((~imm & 0x1f) + 1) << 4;
#if __riscv_xlen == 64
        return imm >> 3; // RV64: In units of 8 bytes
#else
        return imm >> 2;  // RV32: In units of 4 bytes
#endif
    }
...
#endif
        imm = (inst >> 9) & 0x0F; //imm[5:2] already shifted by lowest 2 bits, scaled by 4
        imm = imm | (((inst >> 7) & 0x3)<<4); //imm[7:6]
        /* The unit is size_t, So we don't have to move 3 bits to the left */
#if __riscv_xlen == 64
        return imm >> 1; // RV64: in 8-byte units
#else
        return imm;  // RV32: in 4-byte units
#endif
    }
...
void rvbacktrace_info(uint32_t *uSP, uint32_t *uPC)
{
    ...
    SP = uSP;
    PC = (char *)uPC;
    int   ret;    // save PC value first for addr2line use
