...
#include "app_buffer.h"
    ...
    /*
 * PAST need set 64 bytes
 */
    #if 0
        #define ACL_CONN_MAX_RX_OCTETS    64
        #define ACL_CENTRAL_MAX_TX_OCTETS 64
...
//_attribute_iram_noinit_data_  u8  app_pawr_sync_rsp_buf[APP_PAWR_SYNC_RSP_DATA_LENGTH * APP_PAWR_SYNC_SETS_NUMBER] = {0};
...
int app_le_ext_adv_report_event_handle(u8 *p, int evt_data_len)
{
    ...
    if (pExtAdvRpt->num_reports != 1) {
        tlkapi_printf(APP_PAWR_EVT_LOG_EN, "[APP][EVT] rpt evt combine %d", &pExtAdvRpt->num_reports);
    }
    ...
    for (int i = 0; i < pExtAdvRpt->num_reports; i++) {
        ...
        if (pExtAdvInfo->event_type & EXTADV_RPT_EVT_MASK_LEGACY) {
            //tlkapi_printf(APP_LOG_EN,"bbb:%d", pExtAdvRpt->num_reports);
        } else {
    #if METHOD_ONE_EN
    ...
    #else
            u8 permit_create_flag = 0;

            tlkapi_printf(APP_PAWR_EVT_LOG_EN, "[APP][EVT] sync PDA mac :%x,%x,%x,%x,%x,%x", pExtAdvInfo->address[0], pExtAdvInfo->address[1], pExtAdvInfo->address[2], pExtAdvInfo->address[3], pExtAdvInfo->address[4], pExtAdvInfo->address[5]);
            u8 cmp_addr[6] = {0x99, 0x99, 0x99, 0x99, 0x99, 0x11};
            if (!compare_reject_flag && !smemcmp(pExtAdvInfo->address, cmp_addr, 6)) {
                permit_create_flag = 1;
            }
    #endif
                if (permit_create_flag) {
                    tlkapi_printf(APP_PAWR_EVT_LOG_EN, "[APP][EVT] 222 :%02X", permit_create_flag);
                    if (pExtAdvInfo->perd_adv_inter != PERIODIC_ADV_INTER_NO_PERIODIC_ADV) {
                        ...
                        tlkapi_printf(APP_PAWR_EVT_LOG_EN, "[APP][EVT] blc_ll_periodicAdvertisingCreateSync error code :%02X", status);

                        if (status == BLE_SUCCESS) {
                            compare_reject_flag = 1;
                        }
                    }
                }
        }
...
int app_le_connection_complete_event_handle(u8 *p)
{
    ...
    tlkapi_printf(APP_PAWR_EVT_LOG_EN, "[APP][EVT] le_connection_complete connHandle:%04X mac:%02X %02X %02X %02X %02X %02X", pConnEvt->connHandle, pConnEvt->peerAddr[0], pConnEvt->peerAddr[1], pConnEvt->peerAddr[2], pConnEvt->peerAddr[3], pConnEvt->peerAddr[4], pConnEvt->peerAddr[5]);

    if (pConnEvt->status == BLE_SUCCESS) {
        tlkapi_send_string_data(APP_LOG_EN, "[APP][EVT] connHandle", &gConnHandle, 2);
...
int app_disconnect_event_handle(u8 *p)
{
    ...
    tlkapi_printf(APP_PAWR_EVT_LOG_EN, "[APP][EVT] app Disconnect event connHandle:%04X reason:%02X", pDisConn->connHandle, pDisConn->reason);
    ...
    tlkapi_send_string_data(APP_PAWR_EVT_LOG_EN, "[APP][EVT] enable adv", 0, 0);
...
int app_le_connection_update_complete_event_handle(u8 *p)
{
    ...
    tlkapi_send_string_data(APP_PAWR_EVT_LOG_EN, "[APP][EVT] Connection Update Event", &pUpt->connHandle, 8);
...
{
    hci_le_periodicAdvSyncEstablishedEvtV2_t *pExt = (hci_le_periodicAdvSyncEstablishedEvtV2_t *)p;
    tlkapi_send_string_data(1, "pda sync acad info", &pExt->num_subevent, 4);
    u8 sync_subevent_num = 1;
    u8 sync_subevent[1]  = {2}; //now sync the subevent 2

    if (0 && pExt->status == 0) {
        u8 status = blc_hci_le_setPeriodicSyncSubevent(pExt->syncHandle, 0, sync_subevent_num, sync_subevent);
        if (status != 0) {
            tlkapi_printf(APP_PAWR_EVT_LOG_EN, "[APP][EVT] PAwR sync set sync subevent error");
        } else {
            tlkapi_printf(APP_PAWR_EVT_LOG_EN, "[APP][EVT] PAwR sync set sync subevent OK");
        }
    }


    tlkapi_printf(APP_PAWR_EVT_LOG_EN, "[APP][EVT]periodic_adv_sync_established syncHandle:%04X mac:%02X %02X %02X %02X %02X %02X", pExt->syncHandle, pExt->advAddr[0], pExt->advAddr[1], pExt->advAddr[2], pExt->advAddr[3], pExt->advAddr[4], pExt->advAddr[5]);

    u8 status = blc_ll_setExtScanEnable(BLC_SCAN_DISABLE, DUP_FILTER_DISABLE, SCAN_DURATION_CONTINUOUS, SCAN_WINDOW_CONTINUOUS);
    if (!status) {
        tlkapi_printf(APP_PAWR_EVT_LOG_EN, "[APP][EVT] extended scan disable OK");
    }
    return 0;
}
 ...
 * @return
 */
int app_le_periodic_adv_sync_transfer_received_event_handle(u8 *p)
{
    hci_le_periodicAdvSyncTransferRcvdEvt_V2_t *pPastEvt = (hci_le_periodicAdvSyncTransferRcvdEvt_V2_t *)p;
    tlkapi_send_string_data(APP_PAWR_EVT_LOG_EN, "[APP][EVT] PAST pawr info", &pPastEvt->num_subevt, 4);

    u8 sync_subevent_num = 1;
    u8 sync_subevent[1]  = {2}; //now sync the subevent 2


    if (pPastEvt->status == 0) {
        u8 status = blc_hci_le_setPeriodicSyncSubevent(pPastEvt->syncHandle, 0, sync_subevent_num, sync_subevent);

        if (status != 0) {
            tlkapi_printf(APP_PAWR_EVT_LOG_EN, "[APP][EVT] PAST EVT set sync subevent error");
        } else {
            tlkapi_printf(APP_PAWR_EVT_LOG_EN, "[APP][EVT] PAST EVT set sync subevent OK");
        }
    }


    tlkapi_printf(APP_PAWR_EVT_LOG_EN, "[APP][EVT] periodic_adv_sync_established syncHandle:%04X mac:%02X %02X %02X %02X %02X %02X", pPastEvt->syncHandle, pPastEvt->advAddr[0], pPastEvt->advAddr[1], pPastEvt->advAddr[2], pPastEvt->advAddr[3], pPastEvt->advAddr[4], pPastEvt->advAddr[5]);
...
{
    hci_le_periodicAdvReportEvtV2_t *pPdaReport = (hci_le_periodicAdvReportEvtV2_t *)p;
    ...
    u16 req_pawrEvtCnt = pPdaReport->paEventCounter + 10; //now still not know the meaning of the value.
    u8  req_subevent   = pPdaReport->subevent;            //
    u8  rsp_subevent   = pPdaReport->subevent;
    u8  rsp_slotIdx    = 2;

    tlkapi_send_string_data(APP_PAWR_EVT_LOG_EN, "[APP][EVT] pda report v2,pawr,req&rsp sub", &req_subevent, 1);

    static u8 numCnt = 0;
    numCnt++;
    rsp_dataBuff[0] = numCnt;

    u8 status = blc_hci_le_setPAwRsync_rspData(pPdaReport->syncHandle, req_pawrEvtCnt, req_subevent, rsp_subevent, rsp_slotIdx, RSP_DATA_LEN, rsp_dataBuff);
    if (status != 0) {
        tlkapi_printf(APP_PAWR_EVT_LOG_EN, "[APP][EVT] PAwR sync set response data error");
    } else {
        //tlkapi_printf(APP_PAWR_EVT_LOG_EN, "[APP][EVENT]PAwR sync set response data OK");
    }
...
int app_le_periodic_adv_sync_lost_event_handle(u8 *p)
{
    ...
    tlkapi_printf(APP_PAWR_EVT_LOG_EN, "[APP][EVT] periodic_adv_sync_lost syncHandle:%04X EventCode:%02X", pExt->syncHandle, pExt->subEventCode);
...
int app_le_enhanced_connection_complete_event_handle(u8 *p)
{
    ...
    gConnHandle = pConnEvt->connHandle;
    tlkapi_printf(APP_PAWR_EVT_LOG_EN, "[APP][EVT] Connection complete event, %d", gConnHandle);
...
int app_controller_event_callback(u32 h, u8 *p, int n)
{
    if (h & HCI_FLAG_EVENT_BT_STD) //Controller HCI event
    {
        ...
        } else if (evtCode == HCI_EVT_LE_META)         //LE Event
        {
            ...
            else if (subEvt_code == HCI_SUB_EVT_LE_PERIODIC_ADVERTISING_SYNC_ESTABLISHED_V2) //periodic advertise sync established. extended scan
            {
                tlkapi_printf(APP_PAWR_EVT_LOG_EN, "[APP][EVT] periodic adv sync established V2");

                app_le_periodic_adv_sync_established_event_handle(p);
            } else if (subEvt_code == HCI_SUB_EVT_LE_PERIODIC_ADVERTISING_SYNC_TRANSFER_RECEIVED_V2) { //PAST Event v2, indicate already established

                tlkapi_printf(APP_PAWR_EVT_LOG_EN, "[APP][EVT] past established V2");

                app_le_periodic_adv_sync_transfer_received_event_handle(p);
            }
            //------hci le event: LE periodic advertising report event-------------------------------
            else if (subEvt_code == HCI_SUB_EVT_LE_PERIODIC_ADVERTISING_REPORT)        // periodic advertise report V1
            {
                tlkapi_printf(APP_PAWR_EVT_LOG_EN, "[APP][EVT] periodic advertise report event v1");
            } else if (subEvt_code == HCI_SUB_EVT_LE_PERIODIC_ADVERTISING_REPORT_V2) { // periodic advertise report V2
