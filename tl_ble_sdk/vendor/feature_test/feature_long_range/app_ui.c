    ...
    else if(kb_event.cnt == 1)
    {
        if(key0 >= CR_VOL_UP )  //volume up/down
        {
            ...
            if(key0 == CR_VOL_UP){      //volume up
                ...
                extern void app_switch_test_phy(void);
                app_switch_test_phy();

                #if TEST_ROLE ==  TEST_ROLE_PERIPHR
                    extern void app_switch_adv_info(void);
                    app_switch_adv_info();
                #elif TEST_ROLE ==  TEST_ROLE_CENTRAL
                    extern void app_switch_scan_info(void);
                    app_switch_scan_info();
                #endif
            }
            else if(key0 == CR_VOL_DN){ //volume down
                consumer_key = MKEY_VOL_DN;
                extern void app_switch_test_mode(void);
                app_switch_test_mode();
            ...
            for(int i=ACL_CENTRAL_MAX_NUM; i < (ACL_CENTRAL_MAX_NUM + ACL_PERIPHR_MAX_NUM); i++){ //peripheral index is from "ACL_CENTRAL_MAX_NUM" to "ACL_CENTRAL_MAX_NUM + ACL_PERIPHR_MAX_NUM - 1"
                if(conn_dev_list[i].conn_state){
                    central_unpair_enable = conn_dev_list[i].conn_handle;
                    break;
                }
            }
            #elif TEST_ROLE ==  TEST_ROLE_CENTRAL
