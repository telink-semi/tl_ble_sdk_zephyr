...
_attribute_no_inline_ void user_init_normal(void)
{
    ...
    #ifdef APP_CURRENT_TEST_ADV
    switch (app_power_cfg.pdu_len) {
    case POWER_CFG_PDU_LEN_12BYTES:
        blc_ll_setAdvData(tbl_advData_12bytes, sizeof(tbl_advData_12bytes));
        break;
    case POWER_CFG_PDU_LEN_31BYTES:
        blc_ll_setAdvData(tbl_advData_31bytes, sizeof(tbl_advData_31bytes));
        break;
    default:
        break;
    }
    blc_ll_setAdvParam(ADV_INTERVAL_1S, ADV_INTERVAL_1S, ADV_TYPE_NONCONNECTABLE_UNDIRECTED, OWN_ADDRESS_PUBLIC, 0, NULL, BLT_ENABLE_ADV_ALL, ADV_FP_NONE);
    #elif defined(APP_CURRENT_TEST_CONN)
    blc_ll_setAdvData(tbl_advData, sizeof(tbl_advData));
    blc_ll_setScanRspData(tbl_scanRsp, sizeof(tbl_scanRsp));
    blc_ll_setAdvParam(ADV_INTERVAL_40MS, ADV_INTERVAL_40MS, ADV_TYPE_CONNECTABLE_UNDIRECTED, OWN_ADDRESS_PUBLIC, 0, NULL, BLT_ENABLE_ADV_ALL, ADV_FP_NONE);
    #endif

    blc_ll_setAdvEnable(BLC_ADV_ENABLE); //ADV enable
    blc_ll_setMaxAdvDelay_for_AdvEvent(MAX_DELAY_0MS);

    switch(app_power_cfg.tx_power)
    {
    case POWER_CFG_CCLK_0dBm:
        rf_set_power_level_index(RF_POWER_INDEX_P0p08dBm);
        break;
    case POWER_CFG_CCLK_4dBm:
        rf_set_power_level_index(RF_POWER_INDEX_P4p13dBm);
        break;
    case POWER_CFG_CCLK_8dBm:
        rf_set_power_level_index(RF_POWER_INDEX_P8p03dBm);
        break;
    case POWER_CFG_CCLK_10dBm:
        rf_set_power_level_index(RF_POWER_INDEX_P10p00dBm);
        break;
    default: break;
    }
    ...
    tlkapi_send_string_data(APP_LOG_EN, "[APP][INI] current demo init", 0, 0);

    #ifdef APP_CURRENT_TEST_ADV
    tlk_printf("test_adv: cclk: %x, tx_power: %x, pdu_len: %x\r\n",
               app_power_cfg.cclk,
               app_power_cfg.tx_power,
               app_power_cfg.pdu_len);
    #elif defined(APP_CURRENT_TEST_CONN)
    tlk_printf("test_conn: cclk: %x, tx_power: %x\r\n",
               app_power_cfg.cclk,
               app_power_cfg.tx_power);
    #endif
}
...
{
    //    static u32 tick1 = 0;
    //    if(clock_time_exceed(tick1, 3000 * 1000))   // per 3s, deep sleep
    //    {
    //        tick1 = clock_time();
    //
    //        blc_ll_setAdvEnable(BLC_ADV_DISABLE);  //ADV enable
    //
    //        cpu_sleep_wakeup(DEEPSLEEP_MODE, PM_WAKEUP_TIMER, clock_time() + 117 * SYSTEM_TIMER_TICK_1S);
    //    }
    ////////////////////////////////////// BLE entry /////////////////////////////////
    blc_sdk_main_loop();
