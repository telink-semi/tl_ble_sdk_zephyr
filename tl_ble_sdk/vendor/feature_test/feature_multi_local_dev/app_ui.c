...
_attribute_ble_data_retention_ int del_dev_index = LOCAL_DEVICE_INDEX_0; //0~3 for device 1~4
...
void key_change_proc(void)
{
    ...
    if (kb_event.cnt == 2)     //two key press
    {
        ...
        if (key0 >= CR_VOL_UP) //volume up/down
        {
            ...
            {
                printf("Delete bonding info: %d\r\n", del_dev_index);
                //              blc_smp_deleteBondingInfo_by_device_index(del_dev_index);
                del_dev_index++; /* the index point to the next one. */
                if (del_dev_index > LOCAL_DEVICE_INDEX_3) {
                    del_dev_index = LOCAL_DEVICE_INDEX_0;
                }
            } else if (key0 == BTN_UNPAIR) //Manual un_pair triggered by Key Press
            {
                extern int app_dev_index;
                app_dev_index++;           /* the index point to the next one. */
                if (app_dev_index > LOCAL_DEVICE_INDEX_3) {
                    app_dev_index = LOCAL_DEVICE_INDEX_0;
                }
                printf("Switch to nxt slaveDevIdx:%d\r\n", app_dev_index);
                blc_ll_setCurrentLocalDevice_by_index(app_dev_index);
